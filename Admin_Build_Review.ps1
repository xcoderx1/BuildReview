...
function Invoke-Review {
    ...
    $tab=[char]9
    $descMap=@{
        "Secure Boot"="Secure Boot was not enabled, allowing unsigned bootloaders and boot-level rootkits to persist."
        "TPM"="A Trusted Platform Module was not detected or not enabled, weakening hardware-backed security features."
        "TPM version"="The TPM version is below 2.0, required for modern security features including BitLocker and Credential Guard."
        "UEFI boot mode"="The system uses Legacy BIOS boot mode, preventing Secure Boot and Virtualization Based Security."
        "Virtualization Based Security"="Virtualization Based Security was not enabled, leaving credential isolation unprotected."
        "BitLocker"="Full disk encryption was not enabled, allowing data access via physical disk removal or boot attacks."
        "Encryption strength"="The BitLocker encryption algorithm is weaker than the recommended XTS-AES-256 standard."
        "Suspended BitLocker"="BitLocker protection was suspended on one or more volumes, leaving data unprotected until resumed."
        "Windows Update status"="The system has not been patched recently, leaving known vulnerabilities unaddressed."
        "Vulnerable"="End-of-life or known vulnerable software was detected which may contain unpatched security flaws."
        "Writable service binaries"="Running service executables were writable by non-admin users, enabling privilege escalation."
        "Unquoted service paths"="Unquoted service paths with spaces were found, allowing executable hijacking via path interception."
        "Weak service DACLs"="Weak access controls on services allow non-admin users to modify service configuration."
        "AlwaysInstallElevated"="AlwaysInstallElevated was enabled, allowing any user to install MSI packages with SYSTEM privileges."
        "Remote access tools"="Unapproved remote management tools were detected which could provide persistent unauthorized access."
        "All Users Startup"="The All Users startup folder was writable by standard users, enabling persistence at logon."
        "Writable Program Files"="Subdirectories within Program Files were writable by standard users, enabling DLL hijacking."
        "Writable system PATH"="Directories in the system PATH were writable by standard users, enabling DLL hijacking attacks."
        "Writable C"="The root of C: drive was writable by standard users, allowing file placement for exploitation."
        "Deployment files"="Deployment files (Unattend.xml/Sysprep) were found which may contain cleartext credentials."
        "SAM/SYSTEM"="Backup copies of SAM/SYSTEM hives were accessible, potentially allowing offline password extraction."
        "shadow copies"="Volume shadow copies were present which could allow access to previous versions of sensitive files."
        "GPP cpassword"="Group Policy Preference files containing reversibly encrypted credentials were found."
        "Accessibility"="Accessibility tool binaries or IFEO entries were modified, indicating potential login screen backdoor."
        "Real-Time Protection"="Windows Defender real-time protection was not enabled, leaving the system without active malware scanning."
        "Tamper Protection"="Defender Tamper Protection was disabled, allowing attackers to modify security settings."
        "Behavior Monitoring"="Defender behavior monitoring was disabled, reducing detection of fileless attacks."
        "Signature age"="Antivirus definitions are outdated, reducing detection of recently identified malware."
        "PUA protection"="Potentially Unwanted Application detection was not enabled in Windows Defender."
        "MAPS cloud"="Microsoft cloud-based protection was not configured in Windows Defender."
        "ASR rules"="Insufficient Attack Surface Reduction rules were active, leaving common attack vectors unmitigated."
        "Defender exclusions"="Excessive Defender exclusions were configured, creating blind spots malware could exploit."
        "Domain firewall"="Windows Firewall was not enabled for the Domain network profile."
        "Private firewall"="Windows Firewall was not enabled for the Private network profile."
        "Public firewall"="Windows Firewall was not enabled for the Public network profile."
        "Domain inbound"="The Domain profile firewall was not configured to block inbound connections by default."
        "Private inbound"="The Private profile firewall was not configured to block inbound connections by default."
        "Public inbound"="The Public profile firewall was not configured to block inbound connections by default."
        "Domain logging"="Firewall logging was not enabled for the Domain profile, reducing blocked connection visibility."
        "Private logging"="Firewall logging was not enabled for the Private profile."
        "Public logging"="Firewall logging was not enabled for the Public profile."
        "LSASS RunAsPPL"="LSASS was not running as a Protected Process, allowing credential dumping from memory."
        "Credential Guard"="Credential Guard was not enabled, leaving domain credentials vulnerable to theft attacks."
        "WDigest"="WDigest authentication was enabled, causing plaintext passwords to be stored in LSASS memory."
        "AMSI"="No Anti-Malware Scan Interface providers were registered, reducing script-based attack detection."
        "Sysmon"="Sysmon was not installed, limiting endpoint detection and forensic investigation capability."
        "SmartScreen"="Windows SmartScreen was not enabled, reducing protection against malicious downloads."
        "USB storage"="USB mass storage was not disabled, allowing data exfiltration via removable drives."
        "Device install"="Device installation restrictions were not configured, allowing unauthorized hardware connections."
        "AutoRun"="AutoRun was not fully disabled, allowing malicious code to execute when removable media is inserted."
        "DMA protection"="Kernel DMA protection was not configured, potentially allowing direct memory access attacks."
        "Removable storage write"="Write access to removable storage was not restricted, enabling data exfiltration to USB."
        "BitLocker To Go"="BitLocker encryption was not required for removable storage devices."
        "Password history"="Password history enforcement was insufficient, allowing reuse of previous passwords."
        "password age"="Password age policy was not configured per security best practice."
        "password length"="Minimum password length was below the recommended 14 characters, weakening brute-force resistance."
        "Password complexity"="Password complexity requirements were not enabled, allowing weak passwords."
        "Reversible encryption"="Passwords were stored with reversible encryption, equivalent to storing them in plaintext."
        "Lockout duration"="Account lockout duration was insufficient, allowing rapid brute-force password guessing."
        "Lockout threshold"="Account lockout threshold was not configured, allowing unlimited failed password attempts."
        "Lockout reset"="Account lockout counter reset was too short, reducing lockout policy effectiveness."
        "Blank password"="User accounts with blank passwords were found, allowing unauthenticated access."
        "Stale accounts"="Accounts inactive for over 90 days were found enabled, increasing the attack surface."
        "Local admin"="More than two accounts have local administrator privileges, increasing credential compromise risk."
        "Credential Manager"="Stored credentials were found in Credential Manager which could be extracted by attackers."
        "Guest account"="The built-in Guest account was enabled, providing an unauthenticated access vector."
        "Block Microsoft accounts"="Microsoft account authentication was not blocked on this system."
        "blank password"="The policy to limit blank password use to console logon only was not enabled."
        "UAC"="User Account Control was not optimally configured, weakening the privilege elevation boundary."
        "Admin Approval"="UAC Admin Approval Mode was not optimally configured for the secure desktop."
        "consent prompt"="The UAC consent prompt was not configured to require consent on the secure desktop."
        "Standard user prompt"="UAC was not configured to automatically deny elevation requests from standard users."
        "Secure desktop"="UAC was not using the secure desktop for elevation prompts, allowing UI manipulation attacks."
        "Detect installation"="UAC application installation detection was not enabled."
        "UIAccess"="UAC was not restricted to elevate UIAccess applications only from secure locations."
        "Signed executables"="UAC was not configured to only elevate signed and validated executables."
        "Virtualize write"="UAC file and registry virtualization was not enabled."
        "AutoLogon"="Cleartext credentials were stored in Windows AutoLogon registry keys."
        "Cached domain"="More than four cached domain logon credentials were configured."
        "audit subcategory"="Advanced audit policy subcategory settings were not enforced over legacy audit policies."
        "display last"="The last signed-in username was displayed at logon, revealing valid account names to attackers."
        "CTRL+ALT+DEL"="CTRL+ALT+DEL was not required at logon, making the system susceptible to logon spoofing."
        "inactivity limit"="No machine inactivity timeout was configured, leaving unattended sessions unlocked indefinitely."
        "Logon message"="No legal notice or logon warning banner was configured before authentication."
        "Smart card removal"="No action was configured for smart card removal, leaving sessions accessible."
        "token privileges"="Dangerous token privileges were assigned that could enable privilege escalation to SYSTEM."
        "SYSTEM tasks"="Scheduled tasks running as SYSTEM had writable binary paths, enabling privilege escalation."
        "HKLM Run"="HKLM Run registry key was writable by non-administrators, allowing persistent malware."
        "IFEO"="Image File Execution Options debugger entries were found, indicating executable hijacking."
        "WMI persistence"="WMI event consumers were found which could indicate persistent malware or backdoors."
        "KnownDLLs"="Common DLLs were not in KnownDLLs registry, enabling DLL search order hijacking."
        "Named pipes"="The spoolss named pipe was accessible, potentially enabling PrintSpoofer privilege escalation."
        "COM hijacking"="COM object hijacking entries were detected in the current user registry."
        "AppLocker"="Application whitelisting via AppLocker was not configured, allowing arbitrary executable execution."
        "PowerShell v2"="PowerShell v2 was enabled, which bypasses AMSI, Script Block Logging, and Constrained Language Mode."
        "SMB server sign"="SMB server signing was not required, enabling NTLM relay and man-in-the-middle attacks."
        "SMB client sign"="SMB client signing was not required, enabling NTLM relay attacks against this host."
        "SMB unencrypted"="SMB was configured to allow sending unencrypted passwords to third-party servers."
        "SMBv1"="The legacy SMBv1 protocol was enabled, vulnerable to EternalBlue and other critical exploits."
        "LLMNR"="Link-Local Multicast Name Resolution was enabled, allowing network poisoning and credential capture."
        "NetBIOS"="NetBIOS over TCP/IP was enabled, allowing name resolution poisoning and credential capture."
        "LAN Manager"="LAN Manager authentication level was not set to send NTLMv2 only and refuse LM/NTLM."
        "LDAP client"="LDAP client signing was not required, allowing man-in-the-middle attacks on LDAP traffic."
        "NTLM SSP"="Minimum session security for NTLM SSP was not configured to require NTLMv2 and 128-bit encryption."
        "anon SAM"="Anonymous enumeration of SAM accounts was not restricted, allowing user account enumeration."
        "Everyone includes"="The Everyone group included anonymous users, granting unauthenticated resource access."
        "Null session"="Named pipes accessible to anonymous null sessions were found, exposing system information."
        "remote SAM"="Remote calls to the SAM database were not restricted, allowing remote user enumeration."
        "WinRM"="Windows Remote Management service was running, providing a lateral movement vector."
        "RDP NLA"="Network Level Authentication was not required for Remote Desktop connections."
        "RDP encryption"="Remote Desktop encryption level was not set to High."
        "Hardened UNC"="Hardened UNC paths were not configured, leaving Group Policy traffic vulnerable."
        "CredSSP"="CredSSP encryption oracle remediation was not configured, leaving credentials vulnerable."
        "Non-exportable"="Remote host delegation was not configured to use non-exportable credentials."
        "Wi-Fi stored"="Wi-Fi passwords were recoverable in cleartext from stored wireless profiles."
        "RDP saved"="Saved Remote Desktop connection entries reveal target hosts for lateral movement."
        "PuTTY"="Saved PuTTY SSH session configurations reveal remote access targets."
        "WinSCP"="Saved WinSCP sessions with weakly encrypted credentials were found."
        "WPAD"="Web Proxy Auto-Discovery was configured, exploitable for credential interception."
        "Shares"="Network shares with Everyone access were found, exposing data to all network users."
        "Credential Validation"="Audit logging for credential validation events was not configured per CIS requirements."
        "Group Management"="Audit logging for security group management changes was not configured."
        "User Account Management"="Audit logging for user account management events was not configured."
        "Process Creation"="Audit logging for process creation events was not enabled, limiting forensic visibility."
        "Logon"="Audit logging for logon events was not configured to capture both success and failure."
        "Logoff"="Audit logging for logoff events was not enabled."
        "Special Logon"="Audit logging for special logon events was not enabled."
        "Removable Storage"="Audit logging for removable storage access was not configured."
        "Audit Policy Change"="Audit logging for audit policy modifications was not enabled."
        "Sensitive Privilege"="Audit logging for sensitive privilege usage was not configured."
        "Security State"="Audit logging for security state changes was not enabled."
        "System Integrity"="Audit logging for system integrity events was not configured."
        "Account Lockout"="Audit logging for account lockout events was not configured."
        "Other Logon"="Audit logging for other logon and logoff events was not configured."
        "File Share"="Audit logging for file share access was not configured."
        "Other Object"="Audit logging for other object access events was not configured."
        "Authentication Policy"="Audit logging for authentication policy changes was not configured."
        "Authorization Policy"="Audit logging for authorization policy changes was not configured."
        "MPSSVC"="Audit logging for Windows Firewall rule-level policy changes was not configured."
        "IPsec"="Audit logging for IPsec driver events was not configured."
        "Other System"="Audit logging for other system events was not configured."
        "Security System Extension"="Audit logging for security system extension loading was not configured."
        "Command line"="Process creation events did not include command line arguments, limiting forensic investigation."
        "Script Block"="PowerShell Script Block Logging was not enabled, preventing detection of malicious scripts."
        "Transcription"="PowerShell transcription was not enabled, preventing recording of session activity."
        "Module Logging"="PowerShell Module Logging was not enabled, reducing visibility of cmdlet execution."
        "log size"="Event log maximum size was below recommended thresholds, risking data loss from overflow."
        "log max size"="Event log maximum size was below recommended thresholds, risking data loss from overflow."
        "DEP"="Data Execution Prevention was not fully configured, reducing exploit mitigation capability."
        "ASLR"="Address Space Layout Randomization was not enabled, weakening memory exploit mitigation."
        "Spectre"="Spectre CPU vulnerability mitigations were not explicitly configured."
        "Execution Policy"="PowerShell execution policy was not restricted, allowing unsigned scripts to execute."
        "WSL"="Windows Subsystem for Linux was installed, which can bypass AppLocker, AMSI, and antivirus."
        "WDAC"="Windows Defender Application Control policies were not deployed."
        "BITS"="Active BITS transfer jobs were found which could indicate exfiltration or malware activity."
        "credential files"="Files containing cleartext credentials were found on the filesystem."
        "Browser credential"="Browser credential databases were found which could be extracted by harvesting tools."
        "Lock screen camera"="The lock screen camera was not disabled, potentially allowing unauthorized image capture."
        "Lock screen slideshow"="The lock screen slideshow was not disabled per CIS benchmark."
        "Password reveal"="The password reveal button was not disabled, allowing shoulder-surfing of entered passwords."
        "Consumer experiences"="Microsoft consumer experiences were not disabled, allowing unsolicited app installation."
        "Device metadata"="Device metadata retrieval from the Internet was not disabled."
    }
    ...
    foreach($r in $Script:Results|Where-Object{$_.Status -in @("Fail","Warning")}|Sort-Object Category,Check){
        ...
        $desc=""
        foreach($k in $descMap.Keys){if($pol -match [regex]::Escape($k)){$desc=$descMap[$k];break}}
        if(-not $desc){$desc=$r.Description}
        if(-not $desc -and $r.Exploit){$desc=$r.Exploit}
        if(-not $desc){$desc="This setting was not configured in accordance with security best practice and should be reviewed."}
        $cfg=$r.Actual -replace "`t"," " -replace "`r","" -replace "`n"," "
